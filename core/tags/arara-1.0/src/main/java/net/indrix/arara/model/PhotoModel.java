...
public class PhotoModel {
	...
	/**
	 * The DAO object to be used to retrieve data of photo for identification from database
	 */
	...
	/**
	 * This method retrieves a photo given by the photoId. Only the thumbnail image is retrieved
	 * 
	 * @param photoId The id of the photo to be retrieved
	 * 
	 * @throws DatabaseDownException If the database is down
	 * @throws SQLException If some SQL Exception occurs
	 */
	public Photo retrieveThumbnail(int photoId) throws DatabaseDownException, SQLException {
	...
	/**
	 * This method retrieves the id of all photos from database, for the given the family id
	 *  
	 * @param familyId The id of the family
	 * 
	 * @return An ArrayList object with Integer objects
	...
	public List retrievePhotoIDsForFamily(int familyId)
		...
		logger.debug("PhotoModel.retrievePhotoIDsForFamily | familyId " + familyId);
	...
	/**
	 * This method retrieves the id of all photos from database, for the given the specie id
	 * 
	 * @param specieId The id of the specie
	 * 
	 * @return An ArrayList object with Photo objects
	...
	public List retrievePhotoIDsForSpecie(int specieId)
		throws DatabaseDownException, SQLException {
		logger.debug("PhotoModel.retrievePhotoIDsForSpecie | specieId " + specieId);
	...
	/**
	 * This method retrieves the id of all photos from database, for the given common name id
	 * 
	 * @param commonNameId The id of the common name to be retrieved
	 * @return An ArrayList object with Integer objects
	 * 
	 * @throws DatabaseDownException If the database is down
	 * @throws SQLException If some SQL Exception occurs
	...
	/**
	 * This method retrieves the id of all photos from database, for the given user id
	 * 
	 * @param userId  The id of the user
	 * 
	 * @return An ArrayList object with Photo objects
	 ...
	 * @throws SQLException If some SQL Exception occurs
	 */
	public List retrievePhotoIDsForUser(int userId) throws DatabaseDownException, SQLException {
	 ...
	 * 
	 * @param photo The photo to have its image retrieved from database
	 *
	 * @throws DatabaseDownException If the database is down
	 * @throws SQLException If some SQL Exception occurs
	 */
	public void retrievePhotoImage(Photo photo) throws DatabaseDownException, SQLException {
		logger.debug("PhotoModel.retrievePhotoImage | photoId " + photo.getId());
	 ...
	 * 
	 * @param identification The identification done by a user
	 * 
     * @return true if the identification has been closed, false otherwise
	 * @throws DatabaseDownException If the database is down
	 * @throws SQLException If some SQL Exception occurs
	 * @throws PhotoAlreadyIdentifiedException If the photo has been identified already
	 * 
	 */
	public boolean identifyPhoto(PhotoIdentification identification)
		throws DatabaseDownException, SQLException, PhotoAlreadyIdentifiedException {
		logger.debug("PhotoModel.identifyPhoto : entering method...");
		...
		if (daoIdentification.isPhotoForIdentification(photo.getId())) {
			logger.debug("PhotoModel.identifyPhoto : Identification : " + identification);
			...
			if (photoAuthor.getId() == photoIdentifier.getId()) {
				// photo author has identified photo. Set the specie to it
				...
				// send an email to everyone that identified the photo that the identification has 
				// finished      
				sendFinalEmailToIdentifiers(photoAuthor, identification);
                
                finishedIdentification = true;
			} else {
				// send an email to everyone that identified the photo that the a new photo 
				// identification has been done
				sendEmailToIdentifiers(photoIdentifier, identification);
				// send an email to the photo author that a new identification for his/her photo has 
				// been done
	...
	public List retrieveIdentificationsForPhoto(Photo photo)
		throws DatabaseDownException, SQLException {
		logger.debug("PhotoModel.retrieveIdentificationsForPhoto | photoId " + photo.getId());
	...
	/**
	 * This method notifies the photo author about a new comment added to his/her photo
	 * 
	 * @param photo The photo that received a comment
	 */
	private void notifyPhotoAuthor(Comment c, Photo photo, User user) {
		...
		try {
			...
			CommentsDAO dao = new CommentsDAO();
			List l = dao.retrieveUsersWithCommentsForPhoto(photo.getId(), user.getId());
			...
			while (it.hasNext()) {
				...
				EmailResourceBundle bundle =
					(EmailResourceBundle) EmailResourceBundle.getInstance();
	...
	/**
	 * Format the message to be sent to the photo author about a new identification that has been
	 * done to his/her photo
	 * 
	 * @param user The photo author
	 * @param body The body text template to be formatted
	 * @param comment The comment added to the photo
	...
	/**
	 * This method sends an email to the author of a photo about a new identification that has been
	 * done to his/her photo
	 * 
	 * @param photoAuthor The author of a photo for identification
	 * @param photo The photo under identification
	 */
	private void sendEmailToPhotoAuthor(
		User photoAuthor,
		...
		try {
			...
			// send password to user
			logger.debug("Sending email about notification to photo author with:");
			logger.debug(server);
			logger.debug(fromAdd);

			logger.debug("Photo user = " + photoIdentification.getUser());
			...
			subject = bundle.getString("email.toAuthor.newPhotoIdentification.subject", locale);
			body = bundle.getString("email.toAuthor.newPhotoIdentification.body", locale);
			fromText = bundle.getString("email.toAuthor.newPhotoIdentification.fromText", locale);

			sender.setMessageTextBody(getMessage(photoAuthor, body, photoIdentification));
			sender.setSubject(subject);
			sender.setFromAddress(fromAdd, fromText);
	...
	/**
	 * This method sends an email to everyone that has already identified a photo, about a new 
	 * identification done
	 * 
	 * @param photoIdentifier The new identifier user
	 * @param identification The new identification
	 */
	private void sendEmailToIdentifiers(
		...
		try {
			List listOfUsers =
				daoIdentification.retrieveUsersForIdentification(
					photoIdentification.getPhoto().getId());

			MailClass sender = new MailClass(server);

			// send password to user
			logger.debug("Sending email about notification to all notifiers with:");
			...
			while (it.hasNext()) {
				...
				logger.debug("Going to send to... " + user.getName() + " | " + user.getEmail());
				...
				subject = bundle.getString("email.identification.to.photo.subject", locale);
				body = bundle.getString("email.identification.to.photo.body", locale);
				fromText = bundle.getString("email.identification.to.photo.fromText", locale);
	 ...
	 * identification done
	 * 
	 * @param photoAuthor The new photo author
	 * @param identification The new identification
	 */
	private void sendFinalEmailToIdentifiers(
		User photoAuthor,
		PhotoIdentification photoIdentification) {
		logger.debug("PhotoModel.sendFinalEmailToIdentifiers: entering method...");
		String server = PropertiesManager.getProperty("email.server");
		String fromAdd = PropertiesManager.getProperty("email.from");
		String subject = null;
		String body = null;
