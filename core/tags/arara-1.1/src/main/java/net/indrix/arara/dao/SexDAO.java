 ...
 */
public class SexDAO {
	private static final String SELECT_ALL = "Select * from sex where id > -1";
	...
	public List retrieve() throws DatabaseDownException {
		...
		try {
			logger.debug("SexDAO.retrieve : running SQL " + SELECT_ALL);
			...
			while (rs.next()) {
				...
				Sex sex = new Sex();
				sex.setId(id);
				sex.setSex(description);

				list.add(sex);
				logger.debug("Adding sex: " + sex);
			}
