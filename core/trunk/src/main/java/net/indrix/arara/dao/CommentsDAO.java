...
public class CommentsDAO extends AbstractDAO {
	...
	private static final String INSERT =
		"INSERT INTO user_comments_photo (user_id, photo_id, comment, date) values (?, ?, ?, ?)";

	/**
	 * This sql statement selects all comments for a given photo
	 */
	private static final String SELECT_COMMENTS =
		"SELECT * FROM user_comments_photo where photo_id = ?";

	/**
	 * This sql statement selects all users that had commented an specific photo.
	 */
	private static final String SELECT_USERS_FOR_PHOTO =
		"select u.id, u.login, u.name, u.email, u.language from user_comments_photo c, user u "
	 ...
	 */
	public void insertComment(Comment comment) throws DatabaseDownException, SQLException {
		super.insertObject(comment, INSERT);
	}

	/**
	 * This method retrieves a <code>List</code> object with <code>Comment</code> objects, 
	 ...
	 * @throws SQLException If some SQL Exception occurs
	 */
	public List retrieveComments(Photo photo) throws DatabaseDownException, SQLException {
		this.photo = photo;
	...
	/**
	 * This method retrieves all users that had commented an specific photo. 
	 * The user given by userId is not included in the list
	 * 
	 * @param photoId The id of the photo with comments
	 *  
	 * @return a list with users objects
	 * 
	 * @throws DatabaseDownException If the database is down
	 */
	public List retrieveUsersWithCommentsForPhoto(int photoId, int userId) throws DatabaseDownException {
		...
		try {
			logger.debug(
				"CommentsDAO.retrieveUsersForPhoto : running SQL " + SELECT_USERS_FOR_PHOTO);
            logger.debug("CommentsDAO.retrieveUsersForPhoto : running SQL with " + photoId + "," + userId);
            ...
            logger.debug("CommentsDAO.retrieveUsersForPhoto : adding emails to list...");
			while (rs.next()) {
