...
public class PhotoModel {
	...
	public List retrieveCommentsForPhoto(Photo photo)
			throws DatabaseDownException, SQLException {
		logger.debug("PhotoModel.retrieveCommentsForPhoto | photoId "
				+ photo.getId());
	...
	public void insertComment(Photo photo, User user, String comment)
		...
		c.setUser(user);
		c.setDate(new Date());
		logger.debug("PhotoModel.insertComment : inserting comment to photo "
				+ photo);
		logger.debug("PhotoModel.insertComment : inserting comment on"
				+ c.getDate());
		CommentsDAO dao = new CommentsDAO();
		dao.insertComment(c);

		logger.debug("Notifying user " + photo.getUser().getLogin()
				+ " about new comment.");
	...
	public boolean identifyPhoto(PhotoIdentification identification)
			throws DatabaseDownException, SQLException,
			PhotoAlreadyIdentifiedException {
		...
		if (daoIdentification.isPhotoForIdentification(photo.getId())) {
			logger.debug("PhotoModel.identifyPhoto : Identification : "
					+ identification);
	...
	public List retrieveIdentificationsForPhoto(Photo photo)
			throws DatabaseDownException, SQLException {
		logger.debug("PhotoModel.retrieveIdentificationsForPhoto | photoId "
				+ photo.getId());
	...
	private void sendFinalEmailToIdentifiers(User photoAuthor,
			PhotoIdentification photoIdentification) {
		logger
				.debug("PhotoModel.sendFinalEmailToIdentifiers: entering method...");
		String server = PropertiesManager.getProperty("email.server");
		String fromAdd = PropertiesManager.getProperty("email.from");
		String subject = null;
		String body = null;
