...
public class AbstractIdentificationServlet extends AbstractServlet {
	...
	protected IdentifyPhotoBean handleBean(HttpServletRequest req,
			HttpSession session, String beanKey, List errors) {
		logger
		...
		if (bean == null) {
			logger
					.debug("AbstractIdentificationServlet.handleBean: CREATING bean...");
	...
	protected void retrieveIdentificationsForPhoto(PhotoModel model, Photo photo)
		...
		logger
				.info("IdentifyPhotoServlet.retrieveIdentificationsForPhoto: entering method...");
		...
		if ((identifications == null) || (identifications.isEmpty())) {
			...
			logger.debug(identifications.size()
					+ " identifications found for photo " + photo);
		}
	}
