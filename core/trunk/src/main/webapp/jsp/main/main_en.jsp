<%@ taglib uri="/core" prefix="c"%>
<%@ taglib uri="functions" prefix="f"%>
      <div align="center">
        <table width="100%" border="0" cellspacing="2" height="100%">
          <tr>
            <td>
              <p>Brazilian Birds Database System, version 03.00</p>
              <hr>
              <p>Welcome to <b>Brazilian Birds System</b>. </p>
              <p>The goal of this project is to provide an easy way for those interested in brazilian birds, to 
              share photos or visualize photos from other authors, making this system a Brazilian Birds 
              database.</p>
              <p>Sugestions, critics and ideas are welcome. Send your comments to email 
              <a href="mailto:webmaster@aves.brasil.nom.br">webmaster</a></p>
              <b>This system follows the name convention of CBRO </b>
              <br>
              <br>
              <br>
              <br>
              <br>
              <br>
              <br>
              <br>
            
              <a href="<c:url value="/jsp/language.jsp?lang=pt"/>">Versão em Português</a>
              <hr>
            </td>
          </tr>
          <tr>
            <td>
              <div align="center">
              <a href="http://www.avistarbrasil.com.br" target="_BLANK">
                <img src="/jsp/images/3bannersveio.jpg" align="bottom">
              </a>
              
              <a href="http://www.birdingtop500.com/cgi-bin/topsite/topsite.cgi?user=jeffersonsilva">
                <img src="http://www.birdingtop500.com/cgi-bin/topsite/counter.cgi?user=jeffersonsilva" alt="Birding Top 500 Counter" border=1>
              </a>
              <!--<img src="/jsp/images/PicaPauzinho1.jpg" width="160" height="107" align="bottom">
              <img src="/jsp/images/PicaPauzinho2.jpg" width="160" height="107" align="bottom">-->
              </div>
            </td>
          </tr>
        </table>
      </div>
