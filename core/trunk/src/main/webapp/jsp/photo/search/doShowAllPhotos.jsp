		...
		<table width="100%" border="0" cellspacing="2" height="100%">
			...
			<c:forEach items="${listOfPhotos}" var="photo">
				...
				<img
					...
					align="bottom" /></a> <c:if test="${photo.specie.id != -1}">
					<font size="1" face="Verdana"><br>
					<fmt:message key="family" />: <c:if
						test="${photo.specie.family.subFamilyName == null}">
						<a
							href="<c:url value="/servlet/searchPhotosByFamily?id=${photo.specie.family.id}&action=BEGIN"/>">${photo.specie.family.name}
						</a>
					</c:if> <c:if test="${photo.specie.family.subFamilyName != null}">
						<a
							href="<c:url value="/servlet/searchPhotosByFamily?id=${photo.specie.family.id}&action=BEGIN"/>">${photo.specie.family.name}
						(${photo.specie.family.subFamilyName}) </a></font>
				</c:if> <font size="1" face="Verdana"><br>
				<fmt:message key="specie" />: <a
					href="<c:url value="/servlet/searchPhotosBySpecie?id=${photo.specie.id}&action=BEGIN"/>">${photo.specie.name}
