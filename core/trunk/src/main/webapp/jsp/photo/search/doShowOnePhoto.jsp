...
<table border="1" width="${tableW}%">
	<tr>
		<td align="${imageAlign}"><c:if test="${viewMode == 'viewMode'}">
			<a
				href="<c:url value="/servlet/getPhoto?photoId=${currentPhoto.id}&identification=${identification}"/>"
				target="_blank"> <img:showImage identification="${identification}" />
			</a>
		</c:if> <c:if test="${viewMode == 'identificationMode'}">
			<table>
				<tr>
					<td align="left"><c:import
						url="/jsp/photo/search/doShowPhotoData.jsp" />
					<td align="left"><a
						href="<c:url value="/servlet/getPhoto?photoId=${currentPhoto.id}&identification=${identification}"/>"
						target="_blank"> <img
	...
	<c:if test="${currentPhoto.specie.id != -1}">
		<tr>
			...
			<table width="100%" bgcolor="#005500">
				<tr>
					<td><font size="${fontSize}" face="Verdana" color="#ffffff"><fmt:message
						key="show.one.bird.data.title" /></font></td>
				</tr>
			</table>
			<table width="100%">
				<tr>
	...
	<c:if test="${viewMode == 'viewMode'}">
		<tr>
			<td align="left"><c:import
				url="/jsp/photo/search/doShowPhotoData.jsp" /></td>
		</tr>
	</c:if>
