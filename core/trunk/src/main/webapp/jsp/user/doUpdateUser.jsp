...
<table width="100%" border="0" cellspacing="2" bgcolor="#A6D2D2">
	...
	<tr>
		<td width="20%"></td>
		<td width="80%"><c:if test="${user.emailOnNewPhoto == true}">
			<input type="checkbox" checked name="emailOnNewPhoto" size="16"
				maxlength="16">
			<fmt:message key="user.emailOnNewPhoto" />
		</c:if> <c:if test="${user.emailOnNewPhoto == false}">
			<input type="checkbox" name="emailOnNewPhoto" size="16"
				maxlength="16">
			<fmt:message key="user.emailOnNewPhoto" />
		</c:if></td>
	</tr>
	<tr>
		<td width="20%"></td>
		<td width="80%"><c:if test="${user.emailOnNewIdPhoto == true}">
	...
	<tr>
		<td width="20%"></td>
		<td width="80%"><c:if test="${user.emailOnNewSound == true}">
			<input type="checkbox" checked name="emailOnNewSound" size="16"
				maxlength="16">
			<fmt:message key="user.emailOnNewSound" />
		</c:if> <c:if test="${user.emailOnNewSound == false}">
			<input type="checkbox" name="emailOnNewSound" size="16"
				maxlength="16">
			<fmt:message key="user.emailOnNewSound" />
		</c:if></td>
	</tr>
...
<table width="100%" border="0" cellspacing="2" align="center"
	...
	<tr>
		...
		<div align="left"><input type="SUBMIT"
			value="<fmt:message key="user.update.submit"/>"></div>
		</td>
	</tr>
</table>
<p>&nbsp;</p>
